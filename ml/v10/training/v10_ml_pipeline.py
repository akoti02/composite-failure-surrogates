"""
================================================================================
RP3 V10 MACHINE LEARNING PIPELINE — 101K CALCULIX COMPOSITE SURROGATE MODEL
================================================================================
...
PURPOSE:
    Train ML surrogate models on V10 composite simulation data (101,000 samples).
    CFRP plate with 1-5 crack-like defects, [0/45/-45/90]s layup.
    Predicts stress fields, Tsai-Wu index, Hashin damage modes, and failure.
    Open-source CalculiX solver (V8-V10), cloud-based campaign on GCP.

IMPROVEMENTS OVER V7:
    1. 101,000 samples (vs 891 in V7) — 113x more data
    2. 98 input features (vs 35 in V7) — includes engineered physics features
    3. CalculiX open-source solver (validated to within 2% of Abaqus)
    4. Per-defect localised stress prediction (5 sub-models)
    5. Neural network crossover verification (Grinsztajn et al. 2022)
    6. 11 methodological validation experiments (Appendix G)
    7. PINN achieves R² = 0.904 for peak stress
    8. XGBoost achieves Tsai-Wu F₁ = 0.948, Hashin F₁ = 0.962

NOTE: This script is the V10-configured version of v7_ml_pipeline.py.
    The core ML methodology is identical; only the data path, figure prefix,
    and configuration parameters differ. The V7 pipeline was reused because
    V8-V9 were simulation-only stepping stones (3k and 20k samples) used
    to validate the CalculiX solver and scale up infrastructure.

EXPECTED CSV: results_merged_101k.csv (101,000 rows, 117 columns)
    Input features (98): defect geometry (87), loading (9), laminate (2), interaction (6)
    Regression targets (9): tsai_wu_index, max_mises, max_s11, min_s11, max_s12,
                            max_hashin_ft, max_hashin_fc, max_hashin_mt, max_hashin_mc
    Classification targets (2): failed_tsai_wu, failed_hashin
    Per-defect targets (5): max_mises_defect1..5

USAGE:
    1. pip install catboost shap optuna mapie
    2. Place results_merged_101k.csv in DATA_DIR (or update DATA_PATH)
    3. Run: python v10_ml_pipeline.py
    4. Figures saved as fig_v10_*.png at 300 DPI in figures_v10/
...
# Training settings
K_FOLDS = 5
N_REPEATS = 3          # for repeated k-fold
TEST_SIZE = 0.2
VAL_SIZE = 0.2
BATCH_SIZE = 32
EPOCHS = 300
...
# ==============================================================================
# FEATURE ENGINEERING — DIMENSIONLESS & PHYSICS-BASED
# ==============================================================================

def engineer_features(df):
    """
    Create physics-informed dimensionless features.
    These help the ML models learn scale-invariant relationships.
    """
    df = df.copy()
    n_engineered = 0

    # --- Dimensionless crack lengths (crack_length / plate_width) ---
    ...
    # --- Normalized crack positions (distance from center / plate dimension) ---
    for i in range(1, MAX_DEFECTS + 1):
        x_col = f'defect{i}_x'
        y_col = f'defect{i}_y'
    ...
    # --- Total crack length (sum of all crack lengths, normalized) ---
    ...
    # --- Load ratio (biaxial stress ratio) ---
    ...
    # --- Max crack-to-width ratio (largest single crack / plate width) ---
    ...
    # --- Crack alignment with load (angle between biggest crack and load direction) ---
    for i in range(1, MAX_DEFECTS + 1):
...
def preprocess(df, feature_names, target_name, task='regression'):
    """Extract features and target, apply StandardScaler."""
...
class RegressionNet(nn.Module):
    """Single-output regression network."""
...
class MultiOutputNet(nn.Module):
    ...
    Shared layers capture cross-target correlations, then separate
    heads specialize for each output.
    """
...
class PhysicsInformedLoss(nn.Module):
    """
    MSE loss + physics penalty for impossible predictions.
    Penalizes: negative stress, negative Hashin indices, Tsai-Wu < 0.
    """
    ...
    def forward(self, pred, target):
        ...
        # Penalty for negative predictions (stresses and indices should be >= 0)
...
def train_multioutput_nn(X_train, y_train, X_val, y_val, input_size, n_outputs,
                         epochs=EPOCHS, lr=LEARNING_RATE, patience=EARLY_STOPPING_PATIENCE,
                         verbose=True):
    """Train multi-output network predicting all regression targets at once."""
    train_loader = DataLoader(FEADataset(X_train, y_train, 'regression'),
                              batch_size=BATCH_SIZE, shuffle=True)
    val_loader = DataLoader(FEADataset(X_val, y_val, 'regression'),
                            batch_size=BATCH_SIZE, shuffle=False)
...
def predict_multioutput_nn(model, X):
    """Predict all outputs from multi-output network."""
    model.eval()
...
def mc_dropout_predict(model, X, n_forward=50):
    """
    Monte Carlo Dropout: keep dropout active at inference for uncertainty estimation.
...
def repeated_kfold_cv(X, y, feature_names, target_name, task='regression',
    ...
    """
    Run repeated k-fold CV for all models.
    Returns summary dict and last-fold models.
    """
    print(f"\n{'='*60}")
    ...
    for fold_i, (train_idx, val_idx) in enumerate(rkf.split(X, y)):
        ...
        # Neural network (standard + PINN for regression, standard only for classification)
...
def run_gpr(X_train, y_train, X_test, y_test, target_name):
    ...
    Returns predictions, uncertainty (std), and metrics.
    """
    print(f"\n  GPR for {target_name}...")

    # Subsample if too many points (GPR is O(n^3))
    max_gpr_samples = 500
...
def run_conformal_prediction(X_train, y_train, X_test, y_test, target_name):
    """
    MAPIE conformal prediction: statistically guaranteed prediction intervals.
    Uses CrossConformalRegressor (MAPIE v1.3+ API).
    """
    if not HAS_MAPIE:
...
def plot_data_overview(df, features, detected):
    """Target distributions, class balance, correlation heatmap, n_defects."""
...
def plot_cv_comparison(summary, target_name, task='regression'):
    """Bar chart comparing all models from CV."""
...
def plot_shap_analysis(model, X, feature_names, target_name, model_name='XGBoost'):
    """SHAP beeswarm plot showing feature impact on predictions."""
    if not HAS_SHAP:
...
def plot_mc_dropout_uncertainty(y_true, y_mean, y_std, target_name):
    """Plot MC Dropout uncertainty estimates."""
    sort_idx = np.argsort(y_true)
    y_true_s = y_true[sort_idx]
...
def plot_multi_target_comparison(all_summaries, task='regression'):
    """Compare best model across all targets."""
    metric = 'R2' if task == 'regression' else 'F1'
    ylabel = 'R$^2$ Score' if task == 'regression' else 'F1 Score'
...
def plot_multioutput_nn_comparison(y_true_multi, y_pred_multi, target_names):
    """Compare multi-output NN predictions across all targets."""
...
def main():
    ...
    for target in detected['reg_targets']:
        ...
        # Scatter plot (best model on hold-out)
    ...
    if HAS_OPTUNA and detected['reg_targets']:
        ...
        print(f"{'='*60}")
        # Tune on primary target (tsai_wu_index or first available)
    ...
    # ===================== STEP 7: GPR WITH UNCERTAINTY =====================
    gpr_results = {}
    print(f"\n{'='*60}")
    print("GAUSSIAN PROCESS REGRESSION (with uncertainty)")
    print(f"{'='*60}")

    for target in detected['reg_targets'][:3]:  # Top 3 targets (GPR is slow)
        X, y, scaler = preprocess(df, features, target, task='regression')
        X_tr, X_te, y_tr, y_te = train_test_split(X, y, test_size=TEST_SIZE,
                                                    random_state=RANDOM_SEED)
    ...
    # ===================== STEP 9: MC DROPOUT UNCERTAINTY =====================
    print(f"\n{'='*60}")
    print("MC DROPOUT UNCERTAINTY (Neural Network)")
    print(f"{'='*60}")

    for target in detected['reg_targets'][:2]:  # Top 2 targets
        X, y, scaler = preprocess(df, features, target, task='regression')
        X_tr, X_te, y_tr, y_te = train_test_split(X, y, test_size=TEST_SIZE,
                                                    random_state=RANDOM_SEED)
        nn_split = int(len(X_tr) * 0.8)
    ...
    # ===================== STEP 10: MULTI-OUTPUT NEURAL NETWORK =====================
    multioutput_r2 = None

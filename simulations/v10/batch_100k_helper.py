"""
100K CalculiX composite plate simulations — distributed across multiple GCP VMs.
Each VM runs a slice of the total work defined by --start and --end sim IDs.

Usage:
  python3 batch_100k.py --vm 1 --workers 100 --test 5    # smoke test on VM1
  python3 batch_100k.py --vm 1 --workers 100              # full run on VM1
  python3 batch_100k.py --vm 2 --workers 100              # full run on VM2
  python3 batch_100k.py --vm 3 --workers 100              # full run on VM3
  python3 batch_100k.py --vm 4 --workers 55               # full run on VM4 (smaller)

VM allocation (101,000 sims total):
  VM1: sims     1 - 25,250  (c2d-standard-112, 100 workers)
  VM2: sims 25,251 - 50,500  (c2d-standard-112, 100 workers)
  VM3: sims 50,501 - 75,750  (c2d-standard-112, 100 workers)
  VM4: sims 75,751 - 101,000 (c2d-standard-60,   55 workers)
"""
...
BACKUP_INTERVAL = 500  # backup CSV every N completed sims
MAX_FAIL_RATE = 0.10   # auto-pause if failure rate exceeds 10%
MIN_SIMS_FOR_FAIL_CHECK = 200  # don't check fail rate until this many sims done
...
# =============================================================================
# Worker function — runs ONE simulation
# =============================================================================
def run_single_sim(args):
    ...
    # Create isolated temp directory for this sim to avoid file contention
...
def main():
    ...
    parser.add_argument('--start', type=int, default=0,
                        help='Override start sim ID (default: use VM_RANGES)')
    parser.add_argument('--end', type=int, default=0,
                        help='Override end sim ID (default: use VM_RANGES)')
    parser.add_argument('--tag', type=str, default='',
                        help='Output file tag, e.g. "help1" -> results_help1.csv')
    ...
    tag = args.tag if args.tag else f"vm{vm_id}"
    OUTPUT_CSV = os.path.join(WORK_DIR, f"results_{tag}.csv")
    LOG_FILE = os.path.join(WORK_DIR, f"batch_100k_{tag}.log")
    ...
    # Step 1: Generate ALL samples (same seed on every VM for consistency)
    ...
    # Step 2: Generate ALL crack polygons (same seed on every VM)
    ...
    # Step 4: Build work list — ONLY this VM's range
    work_items = []
    for i, sample in enumerate(all_samples):
        sim_id = i + 1
    ...
    with Pool(processes=n_workers) as pool:
        for i, row in enumerate(pool.imap_unordered(run_single_sim, work_items, chunksize=1)):
            ...
            # Safety check: auto-pause if failure rate too high

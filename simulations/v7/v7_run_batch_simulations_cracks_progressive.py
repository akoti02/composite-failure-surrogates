"""
============================================================
ABAQUS BATCH RUNNER V7 — COMPOSITE PLATE WITH VARIABLE N
JAGGED CRACK-LIKE DEFECTS + PROGRESSIVE DAMAGE
============================================================
BATCH ONLY — no calibration, no validation.
Run v7_calibration.py and v7_validation.py separately first.

Run with (headless, no screenshots):
    abaqus cae noGUI=v7_run_batch_simulations_cracks_progressive.py

Run with (screenshots enabled — requires display):
    abaqus cae script=v7_run_batch_simulations_cracks_progressive.py

Output:
    simulation_results_v7.csv    — ML training data
    v7_batch_log.txt             — per-job detailed log
    v7_screenshots/              — mesh + deformed images (every 25 sims)

Features:
    - Hashin damage evolution (energy-based, linear softening)
    - Damage stabilisation (viscous regularisation)
    - Geometric nonlinearity (nlgeom=ON)
    - CSV row written for EVERY simulation (success or error)
    - Full per-job logging to text file
    - Mesh + deformed-state screenshots every 25 jobs
    - Resume capability (reads existing CSV)
============================================================
"""
...
# --- Fracture energies for Hashin damage evolution (N/mm) ---
GFT = 12.5           # Fibre tension fracture energy
GFC = 12.5           # Fibre compression fracture energy
GMT = 1.0            # Matrix tension fracture energy
GMC = 1.0            # Matrix compression fracture energy
...
def log_banner():
    """Write log header at script start."""
    divider = '=' * 60
    log(divider)
...
def setup_screenshot_quality():
    """Configure Abaqus viewport for highest-quality screenshots."""
...
def take_mesh_screenshot(sim_id, part):
    """Save screenshot of meshed part. Fails gracefully."""
...
def take_deformed_screenshot(sim_id, odb):
    """Save screenshot of deformed shape with von Mises contour."""
    if not SCREENSHOTS_ENABLED:
        return
    try:
        setup_screenshot_quality()
...
def load_completed_ids(filepath):
    """Read existing CSV, return set of successful sim_ids.
    Also strips ERROR rows from the CSV to prevent duplicates on resume."""
    completed = set()
    ...
    if len(lines) <= 1:
        return completed
    # Separate header, good rows, and error rows
    header = lines[0]
    good_rows = []
    for line in lines[1:]:
        ...
        if len(parts) == 0 or not line.strip():
            continue
        if 'ERROR' in line:
            # Drop error rows — they will be retried
            continue
        try:
            ...
            pass
    # Rewrite CSV without error rows
...
# ============================================================
# RUN ONE SIMULATION (with logging + screenshots)
# ============================================================
def run_single_simulation(sample, sim_id, keep_odb=False,
    ...
    try:
        ...
        # Verify key keywords are in the input file
        ...
        # ==== EXTRACT RESULTS ====
        # Pre-initialise all result variables so they have safe
        # defaults if extraction is interrupted partway through.
        max_mises = 0.0
        max_s11 = -1e20
        min_s11 = 1e20
        max_s22 = -1e20
...
# ============================================================
# CSV WRITING (robust — always writes)
# ============================================================
...
def write_error_row(filepath, sim_id, sample, error_msg=''):
    ...
    # min_inter_defect_dist is computable from inputs alone
    vals.append(compute_min_inter_dist(defects))
    # Remaining output columns: 11 stress/hashin + MAX_DEFECTS per-defect
    # mises + 4 trailing (max_disp, n_elements, failed_tw, failed_h)
    # + 3 new (max_sdeg, n_damaged_elements, solver_completed) = 23
...
def main():
    ...
    for i, sample in enumerate(all_samples):
        ...
        # Decide if this sim gets screenshots

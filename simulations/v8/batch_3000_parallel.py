"""
Parallel batch CalculiX simulation script: composite plate simulations with random crack defects.
Uses multiprocessing with 10 workers (configurable) for ~10x speedup.
Same logic as batch_3000.py but runs simulations concurrently.
"""
...
# =============================================================================
# Constants (identical to batch_3000.py)
# =============================================================================
PLATE_L = 100.0
PLATE_W = 50.0
NUM_SAMPLES = 20750
...
CSV_COLUMNS = [
    ...
    # Per-defect engineered features
...
# =============================================================================
# Sampling functions (exact copies from batch_3000.py / Abaqus script)
# =============================================================================
...
# =============================================================================
# Mesh generation (identical to batch_3000.py)
# =============================================================================
...
# =============================================================================
# Write CalculiX .inp file (identical to batch_3000.py)
# =============================================================================
...
# =============================================================================
# Stress parsing (identical to batch_3000.py)
# =============================================================================
...
def compute_defect_features(d, pressure_x, pressure_y):
    """Compute engineered features for a single defect."""
    x, y = d['x'], d['y']
    hl = d['half_length']
    w = d['width']
    ...
    # Ligament ratio: fraction of plate width NOT cut by crack projection
    crack_proj = 2 * hl * abs(sin_a)  # projection onto Y axis
    ligament_ratio = max(0.0, (PLATE_W - crack_proj) / PLATE_W)

    # Analytical SIF estimate: K_I = sigma * sqrt(pi * a) * F(a/W)
    # Using combined biaxial stress normal to crack
    sigma = abs(pressure_x * sin_a**2 + pressure_y * cos_a**2)
    a = hl
    a_over_W = (2 * a) / PLATE_W
    # Finite-width correction factor (Tada, Paris & Irwin approximation)
    if a_over_W < 0.95:
...
def build_row(sim_id, sample, metrics=None, error=False, n_elements=0):
    ...
    # Raw defect parameters + engineered features
    all_sifs = []
    all_ligaments = []
    ...
    for di in range(MAX_DEFECTS):
        ...
        if di < len(sample['defects']):
            ...
            total_crack_area += d['half_length'] * d['width'] * 2  # approximate crack area
        else:
...
# =============================================================================
# Worker function — runs ONE simulation in its own process
# =============================================================================
def run_single_sim(args):
    """Run a single simulation. Called by each worker process."""
    sim_id, sample, polygons = args
    job_name = f"batch3k_sim{sim_id}"
    t0 = time.time()

    # Handle polygon self-intersection (already checked, but just in case)
    if polygons is None:
    ...
    row['_time'] = dt  # internal, not written to CSV
...
def main():
    ...
    # Step 1: Generate ALL samples deterministically
    ...
    # Step 4: Build work list (skip completed)
    work_items = []
    for i, sample in enumerate(all_samples):
        sim_id = i + 1
    ...
    with Pool(processes=NUM_WORKERS) as pool:
        for i, row in enumerate(pool.imap_unordered(run_single_sim, work_items, chunksize=1)):
            ...
            # Write to CSV immediately (only main process touches the file)
            ...
            # Print detail for first 5 and every 100th
